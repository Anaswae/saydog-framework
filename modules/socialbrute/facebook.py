...
def login():
    try:
        ...
        print
        print 'Please logim with facebook account !'
        print '{1}--- Login using email & password'
        print '{2}--- Login using token'
        print
        dog = raw_input('choose@number:~$ ')
        if dog == '1' or dog == '01':
                ...
                if 'session_key' in dog:
                    print
                    prints(g+'[+]'+w+' Login successfully')
                    time.sleep(0.5)
                    open('login.txt', 'w').write(jsl['access_token'])
                    menu()
                elif 'www.facebook.com' in jsl['error_msg']:
                    print
                    prints(r+'[!]'+w+' Sorry, your account has been checkpoint')
                    os.system('rm -f login.txt')
                    print
                    sys.exit()
                else:
                    print
                    prints(r+'[!]'+w+' Login failed, please try again later')
                    print
                    sys.exit()
        else:
...
def users():
    try:
        ...
        print r+'[!]'+w+' Something error, maybe your account has been checkpoint'
    else:
        dog = requests.get('https://graph.facebook.com/me?access_token=' + token)
        ID1 = json.loads(dog.text)
        nama = ID1['name']
        print
        print b+'[+]'+w+' Welcome to facebook bruteforce'
        print b+'[+]'+w+' Login as: '+nama
        print
...
def menu():
        try:
                ...
                users()
                print '{1}--- Auto bruteforce target'
                print '{2}--- Multi bruteforce from file'
                print '{3}--- Bulk facebook checker'
                print '{4}--- Logout sessions'
                print
                print '{0}--- back'
                print
                dog = raw_input('choose@number:-$ ')
                if dog == '1':
                        ...
                        try:
                                ...
                                print
                                dog = raw_input(w+"press enter for exit in this tools")
                                if dog == "":
                                        sys.exit(1)
                                else:
                                        sys.exit(1)
                        ...
                        print
                        print r+'[!]'+w+' Removing access token from /modules/socialbrute/login.txt'
                        time.sleep(3)
                        ...
                        print
                        print r+'[!]'+w+' command not found, please input the number'
                        sys.exit(1)
...
def main():
        try:
                ...
                print r+'[!]'+w+' Oops, something error please try again later'
                print r+'[!]'+w+' Removing login sessions'
                os.system('rm -rf login.txt')
                print
                dog = raw_input('[ enter ]')
                if dog == '':
